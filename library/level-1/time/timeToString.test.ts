...
Deno.test('timeToString', async (test) => {
	await test.step('should return an empty string for 0 time', () => {
		const result = timeToString(0);
		expect(result).toBe('no time');
	});

	await test.step('should correctly convert time to string', () => {
		const testCases = [
			...
			{ time: 123456789, expected: '3 years 11 months 18 days 21 hours 33 minutes 9 seconds' },
		];

import { EcsArchetypeEntity, Game, SurfaceType, byEcsArchetype, tileArchetype } from '@lib';
...
function hallucinateMissingNeighbors(
	...
	return [
		...
		.map(([hallucinatedX, hallucinatedY]) => {
			...
			if (otherNewTiles.find((tile) => tile.equalsMapLocation([hallucinatedX, hallucinatedY, z]))) {
				return null;
			}
...
export async function growTerrainForExcavatedEdges(
	...
	const newTiles = tiles
		.filter(
			(tile) => tile.surfaceType.get() === SurfaceType.OPEN && tile.pathingNeighbours.length < 4,
		)
		.reduce(
